...
describe("Get products test", () => {
  ...
  it("should return a product", async () => {
    ...
    // const oldProducts = await firestoreRepository.collection.viewCollection<ProductsType>("users-products/users-1-products/products");
    const response = await httpClient.post("api/v1/products", newProduct, { headers: { authorization: createAuthorizationHeader(userId) } });
    // const newProducts = await firestoreRepository.collection.viewCollection<ProductsType>("users-products/users-1-products/products");
    // console.log("added products ", newProducts.filter(product => !oldProducts.includes(product)));
    // expect(response.data.length).toEqual(oldProducts.length + 1);
